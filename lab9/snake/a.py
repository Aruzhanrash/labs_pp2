...
def spawn_food(snake_body):
    while True:
        ...
        if (food_x, food_y) not in snake_body:
            weight = random.randint(1, 3)  # Случайный вес еды
            return (food_x, food_y), weight, time.time()  # Добавляем время появления еды
...
def render_food(position, weight):
    pygame.draw.rect(window, FOOD_COLOR, (position[0], position[1], TILE_SIZE, TILE_SIZE))
    show_text(str(weight), position[0] + 5, position[1] + 2)

# Основная игровая функция
def main():
    snake_body = [(100, 100), (90, 100), (80, 100)]
    move_direction = "RIGHT"
    food_pos, food_weight, food_time = spawn_food(snake_body)
    points = 0
    stage = 1
    speed = VELOCITY
    food_lifetime = 7  # Время жизни еды (в секундах)
    ...
    while game_running:
        ...
        # Проверка еды
        if (x, y) == food_pos:
            points += food_weight  # Учитываем вес еды
            food_pos, food_weight, food_time = spawn_food(snake_body)
            if points % 3 == 0:
                stage += 1
        ...
        render_food(food_pos, food_weight)
